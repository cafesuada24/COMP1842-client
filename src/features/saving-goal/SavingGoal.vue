    ...
    <form @submit.prevent="handleSavingGoalSubmit" class="mb-8 bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <div class="mb-4">
        ...
        <input
          ...
          :class="{ 'border-red-500': v$.currentGoal.title.$error }" id="name" type="text" v-model="currentGoal.title"
          placeholder="Enter goal name" @blur="v$.currentGoal.title.$touch()">
      ...
      <div class="mb-4">
        ...
        <input
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          :class="{ 'border-red-500': v$.currentGoal.targetAmount.$error }" id="targetAmount" type="number" step="0.01"
          v-model.number="currentGoal.targetAmount" placeholder="Enter target amount"
      ...
      <div v-for="(goal, index) in savingsGoals" :key="index" class="mb-6 p-4 border rounded">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <p class="text-lg">{{ goal.deadline }}</p>
          </div>
        </div>
        <div class="mb-4">
          ...
          <p class="text-sm text-gray-600 mt-1">Progress: {{ ((goal.currentAmount / goal.targetAmount) * 100).toFixed(2)
            }}%</p>
        </div>
  ...
  methods: {
    async fetchData() {
      try {
        ...
        console.log(savingsGoalsResponse.data);
    ...
    editGoal(index) {
      Object.assign(this.currentGoal, this.savingsGoals[index]);
